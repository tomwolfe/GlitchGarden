 ...
 * 
 * Currently uses mock generation for build compatibility.
 * To enable real AI:
 * 1. Install @huggingface/transformers: npm install @huggingface/transformers
 * 2. Configure next.config.js to exclude it from server bundle
 * 3. Replace the generate() method with actual transformers.js code
 * 4. The mockWorker.ts already demonstrates the full UI flow
 */
...
self.onmessage = async (event: MessageEvent<AIRequest>) => {
  ...
  try {
    if (type === 'init') {
      self.postMessage({
        type: 'ready',
        payload: { status: 'AI Worker ready (using mock generation)!' },
      } as AIResponse);
    }
